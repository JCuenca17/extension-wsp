...
function iniciarSistema() {
  ...
  const css = {
    panel:
      "position:fixed; top:20px; right:20px; width:300px; background:#121212; color:#e0e0e0; font-family:Consolas,sans-serif; font-size:12px; border:1px solid #333; border-radius:4px; box-shadow:0 10px 30px rgba(0,0,0,0.9); z-index:999999; display:flex; flex-direction:column;",
    ...
    input:
      "width:96%; background:#000; border:1px solid #444; color:#fff; padding:8px; margin-top:5px; margin-bottom:10px; font-family:inherit;",
  ...
  function bindEvents() {
    ...
    function clickFn(e) {
      ...
      setTimeout(() => {
        if (sys.active) target.style.outline = "1px dashed #ffd600";
      }, 200);

      const cadsUnicos = new Set();
      sys.queue.forEach((v) => cadsUnicos.add(v.autor));
      els.c.innerText = sys.queue.size;
      els.a.innerText = cadsUnicos.size;
    ...
    els.s.onclick = () => {
      ...
      detenerSeleccion();
      els.s.innerText = "SUBIENDO...";
      els.s.disabled = true;
      els.s.style.background = "#333";
      els.st.innerText = "Conectando...";

      const payload = {
        config: { rootId: sys.rootId },
        datos: Array.from(sys.queue.values()),
      };

      chrome.runtime.sendMessage(
        { action: "uploadToDrive", url: sys.scriptUrl, data: payload },
        (res) => {
          if (res && res.success) {
            let omitidos = 0,
              subidos = 0;
